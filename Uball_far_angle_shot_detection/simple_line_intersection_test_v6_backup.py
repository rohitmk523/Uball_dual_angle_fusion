...
def process_full_game(video_path: str, model_path: str, output_dir: str,
    ...
    # Rename video to match near angle naming
    processed_video_path = output_dir / "processed_video.mp4"
    if output_video_path.exists():
        output_video_path.rename(processed_video_path)
        logger.info(f"✓ Output video renamed to processed_video.mp4")

    # Copy ground truth to output directory if provided
    if ground_truth_path and Path(ground_truth_path).exists():
        import shutil
        gt_dest = output_dir / "ground_truth.json"
        shutil.copy(ground_truth_path, gt_dest)
        logger.info(f"✓ Ground truth copied to output directory")

    # Generate session summary
    session_summary = {
        "game_id": output_dir.name.split('(')[1].split('-')[0] if '(' in output_dir.name else "unknown",
        "angle": output_dir.name.split('-')[1].split(')')[0] if ')' in output_dir.name else "unknown",
        "date": output_dir.name.split('(')[0] if '(' in output_dir.name else datetime.now().strftime("%m-%d"),
        "uuid": output_dir.name.split('_')[-1] if '_' in output_dir.name else "unknown",
        "video_path": str(video_path),
        "model_path": str(model_path),
        "detection_method": "line_intersection_v4",
        "statistics": analyzer.stats,
        "timestamp": datetime.now().isoformat()
    }

    session_summary_path = output_dir / "session_summary.json"
    with open(session_summary_path, 'w') as f:
        json.dump(session_summary, f, indent=2)
    logger.info(f"✓ Session summary created")

    # Compare with ground truth if provided and generate accuracy analysis
    if ground_truth_path and Path(ground_truth_path).exists():
        ...
        for shot in analyzer.detected_shots:
            ...
            if closest_gt:
                ...
                gt_outcome = gt_dict[closest_gt]

                if detected_outcome == gt_outcome:
                    if detected_outcome == "made":
                        matches["correct_made"] += 1
                    else:
                        matches["correct_missed"] += 1
                    matched_shots.append({
                        "timestamp": ts,
                        "ground_truth_timestamp": closest_gt,
                        "outcome": detected_outcome,
                        "status": "correct"
                    })
                else:
                    if detected_outcome == "made":
                        matches["incorrect_made"] += 1
                    else:
                        matches["incorrect_missed"] += 1
                    matched_shots.append({
                        "timestamp": ts,
                        "ground_truth_timestamp": closest_gt,
        ...
        # Count false negatives (GT shots not detected)
        matches["false_negatives"] = len(gt_dict) - len(matched_gt)

        # Calculate metrics
        total_correct = matches["correct_made"] + matches["correct_missed"]
        total_gt = len(gt_dict)
        total_detected = len(analyzer.detected_shots)

        accuracy = (total_correct / total_gt * 100) if total_gt > 0 else 0
        precision = (total_correct / total_detected * 100) if total_detected > 0 else 0
        recall = (total_correct / total_gt * 100) if total_gt > 0 else 0
        f1 = (2 * precision * recall / (precision + recall)) if (precision + recall) > 0 else 0

        accuracy_analysis = {
            "ground_truth_shots": total_gt,
            "detected_shots": total_detected,
            "matches": matches,
            "metrics": {
        ...
        accuracy_analysis_path = output_dir / "accuracy_analysis.json"
        with open(accuracy_analysis_path, 'w') as f:
            json.dump(accuracy_analysis, f, indent=2)

        logger.info(f"✓ Accuracy analysis created")
    ...
    logger.info(f"\nFiles created:")
    logger.info(f"  - detection_results.json")
    logger.info(f"  - session_summary.json")
    if ground_truth_path:
        logger.info(f"  - accuracy_analysis.json")
        logger.info(f"  - ground_truth.json")
    logger.info(f"  - processed_video.mp4")
